...
WGET_OPTS=(--no-verbose --no-check-certificate -U Wget/1.20.3)
...
while read -r url; do
  wget --no-clobber --content-disposition --force-directories --no-host-directories --cut-dirs=1 "${WGET_OPTS[@]}" "${url}"
done <urllist
